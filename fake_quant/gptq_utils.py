...
@torch.no_grad()
def rtn_fwrd(model, dev, args,power_scales=None):
    layers = model.model.layers
    torch.cuda.empty_cache()
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            ...
            quantizer.configure(
                layer_weight_bits, perchannel=True, sym=not(args.w_asym), mse=args.w_clip, quant_func=args.quant_func
            )
            W = subset[name].weight.data

            w = W
            if args.power_scale:
                w = sym_quant_fpe2m2_ps(w,power_scale=power_scale[name.split('.')[-2]])
            else:
