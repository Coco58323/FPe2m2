...
def auto_config_search(module, module_kwargs,input_feat):
    from quant_utils import sym_quant_fpe2m2,sym_quant_fpe2m2_autoscale
    ...
    def _search_module_scale(block, linears2quant: list, x, kwargs={}):
        ...
        weight_scales = torch.cat(
            [fc.weight.abs().max(dim=0, keepdim=True)[0] for fc in linears2quant], dim=0
        )
        weight_scales = weight_scales.mean(dim=0)[0].clamp(min=1e-5)
        for fc in linears2quant:
            best_error = float("inf")
            for quantize_func in candidate_func:
                if quantize_func.__name__ == "sym_quant_fpe2m2_smooth":
                    fc.weight.data = fc.weight.data / weight_scales

                fc.weight.data = quantize_func(fc.weight.data)
                ...
                history.append(loss)
                print(f"Quantize function: {quantize_func.__name__}")
                print(f"Loss: {loss}")
                is_best = loss < best_error
                if is_best:
                    best_error = loss
                    temp_best_func_name = quantize_func.__name__
                gc.collect()
                torch.cuda.empty_cache()
            block.load_state_dict(org_sd)
            best_func_name.append(temp_best_func_name)
            print(f"Best quantization function: {best_func_name}")
        # del best_func_name
        gc.collect()
        torch.cuda.empty_cache()
